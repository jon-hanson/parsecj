...
public class IListTest {
    ...
    public void testEmpty() {
        final IList<Integer> empty = IList.empty();
        Assert.assertEquals("empty equals empty", IList.empty(), empty);
        ...
        Assert.assertEquals("map failed for empty list", IList.empty(), empty.map(x -> null));
        Assert.assertTrue("match failed for empty list", empty.match(n -> false, e -> true));
    }

    @Test
    public void testNonEmpty() {
        final IList<Integer> l = IList.list(1, 2, 3, 4);
        Assert.assertEquals("[1,2,3,4] equals [1,2,3,4]", IList.empty().add(4).add(3).add(2).add(1), l);
        ...
        Assert.assertEquals("map failed for non-empty list", IList.list(-1, -2, -3, -4), l.map(i -> -i));
